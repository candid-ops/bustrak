    ...
    <div class="col-md-7">
        <div class="card content-card">
            <div class="card-header">
                ...
                Edit Route — {{ $route->origin }} → {{ $route->destination }}
            </div>
            <div class="card-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-select rounded-3">
                                <option value="city" {{ old('type',$route->type)=='city'?'selected':'' }}>City</option>
                                <option value="intercity" {{ old('type',$route->type)=='intercity'?'selected':'' }}>Intercity</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Status</label>
                            <select name="is_active" class="form-select rounded-3">
                                <option value="1" {{ old('is_active',$route->is_active)?'selected':'' }}>Active</option>
                                <option value="0" {{ !old('is_active',$route->is_active)?'selected':'' }}>Inactive</option>
                            </select>
                        </div>
                        <div class="col-12">
                            ...
                            <input type="text" name="stops"
                                   class="form-control rounded-3"
                                   value="{{ old('stops', $route->stops ? implode(', ', $route->stops) : '') }}">
                        </div>
                    </div>
